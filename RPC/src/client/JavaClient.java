...
//			Calculator.AsyncClient asyncClient = new Calculator.AsyncClient(factory , new TAsyncClientManager(), transport);

//			perform(client);
			testSyn(client);

			transport.close();
		} catch (TException x/* | IOException x*/) {
			x.printStackTrace();
		}
	}
	...
	private static void testSyn(Calculator.Client client) throws TException {
		int a = 2;
		int b = 1;
